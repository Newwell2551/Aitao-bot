...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isRoleSelectMenu()) {
    if (interaction.customId.startsWith('rs_')) {
      ...
      catch (error) { console.error(error); if (!interaction.replied && !interaction.deferred) await interaction.reply({ content: 'เกิดข้อผิดพลาดตอนเลือกยศ', flags: MessageFlags.Ephemeral }); }
    }
    return;
  }
